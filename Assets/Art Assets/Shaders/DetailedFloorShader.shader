...
Shader "SpriteLamp/Detailed Floor Shader"
{
    Properties
    {
		_TintColor("Tint Color", Color) = (1,1,1,1)
        _MainTex ("Diffuse Texture", 2D) = "white" {}		//Alpha channel is plain old transparency
		_DetailTex("Detail Diffuse Texture", 2D) = "white"{}
    ...
    SubShader
    {
        ...
        {    
            ...
            {
				float2 uv2 = TRANSFORM_TEX(input.uv, _NoiseTex);
                float4 diffuseColour = tex2D(_MainTex, input.uv / _TilingFactor.xy).a * tex2D(_DetailTex, input.uv) * _TintColor *  input.color;
                float4 normalDepth = tex2D(_NormalDepth, input.uv) * smoothstep(-1,1.5,tex2D(_NoiseTex, uv2).r)*2;
		...
		uniform sampler2D _DetailTex;
            ...
            float4 frag(VertexOutput input) : COLOR
            {
                ...
                float4 diffuseColour = tex2D(_MainTex, input.uv / _TilingFactor.xy).a * tex2D(_DetailTex, input.uv) * _TintColor * input.color;
				float4 normalDepth = tex2D(_NormalDepth, input.uv) * tex2D(_NoiseTex, uv2).r;
				float4 specGlossValues = tex2D(_SpecGloss, input.uv);
