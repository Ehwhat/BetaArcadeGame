...
Shader "SpriteLamp/StandardMask"
{
    Properties
    {
    ...
    SubShader
    {
		...
		uniform sampler2D _TintMask;
			...
			UNITY_INSTANCING_BUFFER_START(Props)
				UNITY_DEFINE_INSTANCED_PROP(float4, _TintColour)
            ...
            {
				float4 tintColour = UNITY_ACCESS_INSTANCED_PROP(Props, _TintColour);
                float4 diffuseColour = max(tex2D(_MainTex, input.uv), tex2D(_TintMask, input.uv).r * tintColour) *  input.color;
